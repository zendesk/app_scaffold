...
function bindEvents(app) {
  _.each(app.events, function(fn, key) {
    let func = resolveHandler(app, key),
        splittedKey = key.split(' '),
        event = splittedKey.shift();

    if(splittedKey.length > 0){
      $(document).on(event, splittedKey.join(' '), func);
    } else {
...
BaseApp.prototype = {
  ...
  when: function(...promises) {
    return $.when(...promises.map(promise => {
      // convert native promises to jQuery promises. See https://stackoverflow.com/a/36255846
      if (_.isObject(promise) && !$.isFunction(promise.promise) && $.isFunction(promise.then)) {
        const dfd = $.Deferred();
